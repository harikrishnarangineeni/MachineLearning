...
def preprocess():
    ...
    min_array = np.amin(train_data, axis=0)
    max_array = np.amax(train_data, axis=0)
    ...
    for i in range(count):
        if min_array[i] == max_array[i]:
            excess_features.append(i)
        else:
            active_features.append(i)

    train_data = np.delete(train_data, excess_features, axis=1)
    validation_data = np.delete(validation_data, excess_features, axis=1)
    test_data = np.delete(test_data, excess_features, axis=1)
...
def nnObjFunction(params, *args):
    ...
    target_value = np.zeros((len(new_traindata),10))
...
print(endtime)
executiontime = str(round(time.time() - nowtime, 2))
print(executiontime)
...
#x = np.all(train_data==train_data[0,:] , axis=0)
#y = np.where(x==False)
#selected_features = np.array([])
#selected_features = y[0]

## saving params.pickle
